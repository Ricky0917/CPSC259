...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
  ...
  for (i = 0; i < number_of_candidates; ++i) {
      if (sample_length == candidate_length && strcmp(sample_segment, *(candidate_segments + i)) == 0) {
  ...
  if (has_perfect_match > 0) return;
  //else {
      for (i = 0; i < number_of_candidates; ++i) {

          // Insert your code here - maybe a call to calculate_score?
          int score_temp = 0;
          score_temp = calculate_score(sample_segment, *(candidate_segments + i));

          sprintf(int_buffer, "%d", i + 1);
          sprintf(char_buffer, "%d", score_temp);
          strcat(outputline_buffer, "Candidate number ");
          strcat(outputline_buffer, int_buffer);
          strcat(outputline_buffer, " matches with a score of ");
          strcat(outputline_buffer, char_buffer);
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  for (int i = 0; i <= candidate_length - sample_length +ignore; i+=3) {
      ...
      for (int s = 0; s < sample_length-ignore; s+=3) {
          
            if (strncmp(candidate_segment+s+i, sample_segment+s, 3) == 0) {
                temp_score += 10;
            }
            else if (strcmp( codon_names[get_codon_index(candidate_segment + s + i)], codon_names[get_codon_index(sample_segment + s)]) == 0) {
                //if(codon_names[get_codon_index(sample_segment + s)] == codon_names[get_codon_index(candidate_segment + s +i)])
                //if (strcmp(codon_names[get_codon_index(sample_segment+s)], codon_names[get_codon_index(candidate_segment+s+i)]) == 0)
                temp_score += 5;
            }
            else {
                ...
                while(k<3) {
                    ...
                    if (strncmp(candidate_segment+s+i+k, sample_segment+s+k,1) == 0) {
                        temp_score += 2;
                    }             
                    else if (is_base_pair(candidate_segment[s + i + k], sample_segment[s+k] ) ==1) {
                        temp_score += 1;
                    }
                    else {
                        temp_score += 0;
