...
void process()
{
  ...
  //generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell * *maze, int dimension, int row, int column, char* path);
  /* Calculates and displays required data */
  // INSERT YOUR CODE HERE
  construct_shortest_path_info( paths, paths_found, outputstring);
  //construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer );
  ...
  while ( fgets ( line_buffer, BUFFER, maze_file ) ) {
    for ( column = 0; column < dimension; ++column ) {
      ...
      // maze[row][column].visited = ...
		...
		maze[row][column].visited = line_buffer[column];
	  }
    row++;
  }
	return maze;
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	// INSERT CODE HERE
	*outputbuffer = '\0';
	int length = strlen(pathset[0]);
	int num = 0;
	for (int i = 0; i < numpaths; i++) {
		if (length > strlen(pathset[i])) {
			length = strlen(pathset[i]);
			num = i  ;
		}		
	}
	
	strcat(outputbuffer, "Shortest path: ");
	strcat(outputbuffer, pathset[num]);
	strcat(outputbuffer, "");
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	// INSERT CODE HERE
	*outputbuffer = '\0';
	int cost = path_cost(pathset[0]);
	int num = 0;
	for (int i = 0; i < numpaths; i++) {
		if (cost > path_cost(pathset[i])) {
			cost = path_cost(pathset[i]);
			num = i ;
		}
	}
	char* pathsetbuffer = pathset[num];
	char* pathnum[BUFFER];
	strcat(outputbuffer, "Cheapest path: ");
	strcat(outputbuffer,pathsetbuffer);
	sprintf(pathnum, "\nCheapest path cost: %d", cost);
	strcat(outputbuffer, pathnum);
	strcat(outputbuffer, "");
